package com.laifeng.controls
{
	...
	public class LFExtenrnalInterface
	{
		 ...
		 * 	                  type: 1:聊天 2: 金喇叭
		 *                   msg：消息内容
		 *                   isDm:直播台特有(true/false) 是否开启弹幕
		 */
		public function setChatInfo(chatInfo:Object):void{
			//todo  屏蔽弹幕功能
			Console.log("xx:",chatInfo);
			
			if(!chatInfo.hasOwnProperty("isDm")){
				chatInfo.isDm = true;
			}
			var isOpen:Boolean = chatInfo['isDm'];
			var ce:MEvent = new MEvent(NoticeKey.N_SHOW_BARRAGE_BTN,isOpen);
			Notification.get.notify(ce);
			if(isOpen){
